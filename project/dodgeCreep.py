...
# /usr/local/lib/python3.7/dist-packages/GodotEnv-0.1-py3.7-linux-x86_64.egg/_GodotEnv.cpython-37m-x86_64-linux-gnu.so
...
class dodgeCreepEnv(gym.Env):
	def __init__(self, exec_path, env_path, num_actions=5, num_observations=128*128*3, turbo_mode=False):
		...
		with open("stdout.txt","wb") as out, open("stderr.txt","wb") as err:
			if turbo_mode:
				self.process = subprocess.Popen([exec_path, "--handle", self.handle, "--turbo"], stdout=out, stderr=err)
			else:
	...
	def step(self, action):
		...
		#print("elapsed time of step: ", end - start)
